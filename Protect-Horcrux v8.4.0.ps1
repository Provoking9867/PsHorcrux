...
.DESCRIPTION
    v8.4: TELEMETRY & NUCLEAR WIPE.
    - Added Pre-Flight and Mid-Flight disk space validation failsafes.
    - Added Post-Flight hardware distribution summary telemetry.
    - Rerouted all temp ZIP extraction/creation strictly to $WorkDir.
    - Added Nuclear Wipe option to sanitize the entire working directory.
    - Strict UI input validation for native UWP camera shard recovery.
    - SSSS standard polynomial 0x11D with strict lowercase hex formatting.
    - DoD 5220.22-M 3-Pass Secure Shredder.
.NOTES
...
RECOVERY STEPS:
1. Launch the Horcrux Master Script (v8.4) and click "2. RECOVER KEY".
...
$Main = New-Object Windows.Forms.Form -Property @{Text="Horcrux Master v8.4"; Size="450,715"; StartPosition="CenterScreen"}
...
$Btn5 = New-Object Windows.Forms.Button -Property @{Text="5. NUCLEAR WIPE (KEEP SCRIPT)"; Location="25,380"; Size="380,55"; ForeColor="DarkRed"; Font="Microsoft Sans Serif, 8.25pt, style=Bold"}
$Prog = New-Object Windows.Forms.ProgressBar -Property @{Location="25,450"; Size="380,30"; Step=1}
$Main.Controls.AddRange(@($Disp,$Btn1,$Btn2,$Btn3,$Btn4,$Btn5,$Prog))
...
$Btn1.Add_Click({
    ...
    # Pre-Flight Phase: Storage Estimation
    $rawSize = 0
    $filesToZip = Get-ChildItem $fb.SelectedPath -Recurse -File
    if ($filesToZip) { $rawSize = ($filesToZip | Measure-Object -Property Length -Sum).Sum }
    
    $drv = [System.IO.DriveInfo]::new((Get-Item $WorkDir).Root)
    $estNeeded = $rawSize * ($cfg.n + 2) # Raw + Zip + Enc + (N Shards)
    if ($drv.AvailableFreeSpace -lt $estNeeded) {
        $msg = "CRITICAL: Insufficient Disk Space Estimate.`n`nGenerating $($cfg.n) shards requires an estimated $(Format-Bytes $estNeeded) of free space on $($drv.Name).`n`nYou currently only have $(Format-Bytes $drv.AvailableFreeSpace) available. The vault creation has been safely aborted."
        [Windows.Forms.MessageBox]::Show($msg, "Pre-Flight Error", "OK", "Error")
        return
    }
    ...
    # Temp Consolidation
    $zip = Join-Path $WorkDir "tmp_$($cfg.ID).zip" 
    Compress-Archive -Path "$($fb.SelectedPath)\*" -DestinationPath $zip -Force
    ...
    # Mid-Flight Phase: Exact Storage Validation
    $encSize = (Get-Item $payloadPath).Length
    $finalNeeded = $encSize * $cfg.n
    $drv = [System.IO.DriveInfo]::new((Get-Item $WorkDir).Root)
    if ($drv.AvailableFreeSpace -lt $finalNeeded) {
        Remove-Item $zip, $payloadPath, $vPath, $man -Recurse -Force -ErrorAction SilentlyContinue
        $msg = "CRITICAL: Drive Space Depleted.`n`nThe drive cannot accommodate duplication of the $(Format-Bytes $encSize) payload across $($cfg.n) shards. Temporary files have been deleted to reclaim space."
        [Windows.Forms.MessageBox]::Show($msg, "Mid-Flight Error", "OK", "Error")
        return
    }

    Split-Secret $pw $cfg.n $cfg.k $vPath $cfg.ID $payloadPath
    Remove-Item $zip, $man, $payloadPath -Force
    
    # Post-Flight Phase: Hardware Telemetry Summary
    $fRaw = Format-Bytes $rawSize
    $fEnc = Format-Bytes $encSize
    $fReq = Format-Bytes ($encSize + 51200) # Added 50KB for shard TXT/PNG overhead
    
    $summaryMsg = "Vault $($cfg.ID) Successfully Created!`n`nDATA SUMMARY:`n- Original Folder Size: $fRaw`n- Encrypted Vault Size: $fEnc`n`nHARDWARE DISTRIBUTION:`nYou generated $($cfg.n) shards. Each shard folder contains a full copy of the encrypted vault.`n`n- Storage Needed: $($cfg.n) separate devices.`n- Minimum Capacity: Each device must have at least $fReq of free space."
    [Windows.Forms.MessageBox]::Show($summaryMsg, "Vault Creation Summary", "OK", "Information")
})
...
$Btn2.Add_Click({
    ...
    $BR.Add_Click({ 
        ...
        foreach ($tb in $TBs) {
            $val = $tb.Text.Trim()
            if ([string]::IsNullOrWhiteSpace($val)) { [Windows.Forms.MessageBox]::Show("Validation Error: All $k fields must be filled.", "Validation Error"); return }
            if ($val -notmatch '^(\d+)-([A-Fa-f0-9]+)$') { [Windows.Forms.MessageBox]::Show("Invalid format detected: '$val'`n`nShards must strictly follow the 'Number-Hex' format.", "Validation Error"); return }
            
            $idx = $Matches[1]
            if ($validShards.ContainsKey($idx)) { [Windows.Forms.MessageBox]::Show("Duplicate index detected: Shard #$idx.`n`nYou must provide $k UNIQUE shards.", "Validation Error"); return }
            
            foreach ($v in $validShards.Values) {
                if ($v -eq $val) { [Windows.Forms.MessageBox]::Show("Duplicate shard data detected.`n`nYou must provide $k UNIQUE shards.", "Validation Error"); return }
            }
        ...
        if ($rawLines.Count -lt $k) { [Windows.Forms.MessageBox]::Show("Validation Failed.`nYou need exactly $k unique shards.", "Validation Error"); return }
        ...
        if($script:res){
            ...
            [Windows.Forms.MessageBox]::Show("Reconstruction Failed. The cryptographic math could not resolve the key.", "Error")
        } 
    })
...
$Btn3.Add_Click({
    ...
    $of = New-Object Windows.Forms.OpenFileDialog; if($of.ShowDialog() -eq "OK"){
        # Temp Consolidation
        $tz = Join-Path $WorkDir "dec_$([guid]::NewGuid().Guid.Substring(0,8)).zip" 
        & $Global:OSSL aes-256-cbc -d -salt -pbkdf2 -iter 100000 -in "$($of.FileName)" -out "$tz" -pass "pass:$($Disp.Text)"
...
$Btn5.Add_Click({
    $msg = "NUCLEAR OPTION ACTIVATED:`n`nYou are about to permanently DoD-Shred ALL vaults, shards, unencrypted payloads, and DLL dependencies within:`n$WorkDir`n`nThe script and .sha256 signature file will be spared. Everything else will be irrevocably destroyed.`n`nAre you absolutely sure you want to proceed?"
    if ([Windows.Forms.MessageBox]::Show($msg, "SCORCHED EARTH PROTOCOL", "YesNo", "Warning") -eq "Yes") {
        $items = Get-ChildItem $WorkDir -Recurse -File | Where-Object { $_.FullName -ne $ScriptPath -and $_.FullName -ne $HashFile }
        if ($items) {
            foreach ($f in $items) { Invoke-SecureShred $f.FullName $null }
        }
        Get-ChildItem $WorkDir -Recurse -Directory | Sort-Object -Property @{Expression={$_.FullName.Length}; Descending=$true} | Remove-Item -Force
        [Windows.Forms.MessageBox]::Show("Sanitization complete. The workspace has been neutralized. The script will now exit.", "Nuclear Wipe Complete", "OK", "Information")
